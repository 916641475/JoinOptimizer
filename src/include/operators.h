...
public:
    std::unordered_map<SelectInfo, unsigned> required_select_num_;
    /// The destructor
    ...
    virtual bool require(SelectInfo info, unsigned num){return true;};
    /// Run
    virtual void run() = 0;
...
public:
    ...
    bool require(SelectInfo info, unsigned num)override;
    /// Run
    void run() override;
    /// Get  materialized results
...
private:
    ...
    void Probe(uint64_t *, uint64_t, uint64_t, std::vector<std::vector<uint64_t>>&);
...
public:
    ...
    Checksum(std::shared_ptr<Operator> &input,
        ...
        : input_(input), col_info_(std::move(col_info)) {};
    /// Run
    void run() override;
