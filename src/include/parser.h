...
private:
    ...
    /// relation id to columns
    std::vector<std::map<SelectInfo, unsigned>> relation_bind_need_columns_; 
...
public:
    ...
    const std::vector<std::map<SelectInfo, unsigned>> &need_columns() const {
        return relation_bind_need_columns_;
    }
...
private:
    ...
    SelectInfo parseRelColPair(std::string &raw);
};
