...
void QueryInfo::parseRelationIds(std::string &raw_relations) {
    ...
    relation_bind_need_columns_.resize(relation_ids_.size());
}

SelectInfo QueryInfo::parseRelColPair(std::string &raw) {
    std::vector<unsigned> ids;
    splitString(raw, ids, '.');
    SelectInfo selectinfo(0, ids[0], ids[1]);
    return selectinfo;
}
...
void QueryInfo::parsePredicate(std::string &raw_predicate) {
    ...
    if (isConstant(rel_cols[1])) {
        ...
        ++relation_bind_need_columns_[left_select.binding][left_select];
        ++relation_bind_need_columns_[right_select.binding][right_select];
    }
}
...
void QueryInfo::parseSelections(std::string &raw_selections) {
    ...
    for (auto &raw_select : selection_strings) {
        auto select = parseRelColPair(raw_select);
        selections_.emplace_back(select);
        ++relation_bind_need_columns_[select.binding][select];
    }
}
...
void QueryInfo::resolveRelationIds() {
    ...
    for (auto &s_info_nums : relation_bind_need_columns_){
        for(auto &s_info_num : s_info_nums){
            resolveIds(relation_ids_, const_cast<SelectInfo&>(s_info_num.first));
        }
    }
}
...
void QueryInfo::clear() {
    ...
    filters_.clear();
    selections_.clear();
    relation_bind_need_columns_.clear();
}
